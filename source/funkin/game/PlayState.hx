...
class PlayState extends MusicBeatState
{
        ...
        #if mobile
        /**
         * Controles mobile (touch pad ou hitbox)
         */
        public var mobileControls:MobileControls;
        
        /**
         * Botão de pausa para mobile
         */
        public var pauseButton:FlxSprite;
        
        /**
         * Indicador de FPS para mobile (opcional)
         */
        public var fpsCounterMobile:FlxText;
        
        /**
         * Área de toque para eventos especiais
         */
        public var touchArea:FlxSprite;
        
        /**
         * Flag para indicar se está em modo mobile
         */
...
@:dox(hide) override public function create()
{
        ...
        for (rating in [for (i in ratingManager.ratingData) i.name]) hits.set(rating, 0);
        ...
        // INICIALIZAÇÃO DOS CONTROLES MOBILE
        #if mobile
        if (isMobileMode)
        {
                MobileConfig.init();
                ...
                add(mobileControls);
                createPauseButton();
                adjustTextsForMobile();
                FlxG.mouse.visible = false;
                setupTouchArea();
                if (MobileConfig.getControlMode() == "PAD")
                        MobileUtils.showToast("Use os botões para jogar!", false);
        }
        #end
...
dynamic function updateRatingStuff() {
        ...
        override public function update(elapsed:Float)
        {
                ...
                if (isMobileMode)
                {
                        if (fpsCounterMobile != null && fpsCounterMobile.visible)
                                fpsCounterMobile.text = "FPS: " + FlxG.drawFramerate;
                        
                        if (checkPauseButton() && startedCountdown && canPause && !inCutscene)
                                pauseGame();
                        
                        if (FlxG.touches.list.length >= 3)
                        {
                                var allMoving = true;
                                for (touch in FlxG.touches.list)
                                {
                                        if (!touch.justPressed) allMoving = false;
                                }
                                
                                if (allMoving && mobileControls != null)
                                {
                                        editControlsPosition();
                                }
                        }
                }
                #end
        ...
        public function executeEvent(event:ChartEvent) @:privateAccess {
                ...
                switch(event.name) {
                        ...
                        case "Camera Zoom":
                                ...
                                var name = (event.params[2] == "camHUD" ? "camHUD" : "camGame") + ".zoom";
        ...
        public function gameOver(?character:Character, ?deathCharID:String, ?gameOverSong:String, ?lossSFX:String, ?retrySFX:String) {
                ...
                var charToUse:Character = character.getDefault(opponentMode ? dad : boyfriend);
                ...
                #if mobile
                var gameOverSub = new GameOverSubstate(event.x, event.y, event.deathCharID, event.isPlayer, event.gameOverSong, event.lossSFX, event.retrySFX);
                gameOverSub.onRestart = function() {
                        if (mobileControls != null)
                                mobileControls.visible = true;
                };
                openSubState(gameOverSub);
                #else
        ...
        {
                pauseButton = new FlxSprite(FlxG.width - 60, 20);
                pauseButton.makeGraphic(50, 50, 0xFF000000);
                pauseButton.alpha = 0.5;
                
                pauseButton.pixels.fillRect(new openfl.geom.Rectangle(15, 10, 8, 30), 0xFFFFFFFF);
                pauseButton.pixels.fillRect(new openfl.geom.Rectangle(27, 10, 8, 30), 0xFFFFFFFF);
        ...
        private function adjustTextsForMobile():Void
        {
                scoreTxt.size = 24;
                missesTxt.size = 24;
                accuracyTxt.size = 24;
                ...
                missesTxt.y = healthBarBG.y - 30;
                accuracyTxt.y = healthBarBG.y - 30;
        }

        private function setupTouchArea():Void
        {
                touchArea = new FlxSprite(0, 0);
                touchArea.makeGraphic(FlxG.width, FlxG.height, 0x00000000);
                touchArea.scrollFactor.set();
                touchArea.cameras = [camHUD];
                touchArea.alpha = 0;
                
                touchArea.onDown.callback = function(touch:FlxTouch) {
                        if (touch.x < 100 && touch.y < 100 && touch.justPressed)
                        {
                                if (controls.RESET)
                                        gameOver();
                        }
        ...
        private function checkPauseButton():Bool
        {
                ...
                for (touch in FlxG.touches.list)
                {
                        if (touch.justPressed)
                        {
                                ...
                                if (touchX >= pauseButton.x && touchX <= pauseButton.x + pauseButton.width &&
        ...
        public function showFPSCounter():Void
        {
                if (fpsCounterMobile == null)
                {
                        fpsCounterMobile = new FlxText(10, 10, 0, "FPS: 60", 18);
                        fpsCounterMobile.scrollFactor.set();
                        fpsCounterMobile.cameras = [camHUD];
                        fpsCounterMobile.color = 0xFFFFFF00;
                        fpsCounterMobile.borderStyle = FlxTextBorderStyle.OUTLINE;
                        fpsCounterMobile.borderColor = 0xFF000000;
                        add(fpsCounterMobile);
                }
                
                fpsCounterMobile.visible = true;
                fpsCounterMobile.text = "FPS: " + FlxG.drawFramerate;
        }

        private function editControlsPosition():Void
        {
                paused = true;
                persistentUpdate = false;
                
                var editSubState = new FlxSubState();
                editSubState.persistentUpdate = false;
                
                var instructionText = new FlxText(0, 50, FlxG.width, 
                        "Arraste os controles para reposicionar\nToque com 3 dedos para salvar", 24);
                instructionText.alignment = CENTER;
                instructionText.screenCenter(X);
                instructionText.color = 0xFFFFFFFF;
                instructionText.borderStyle = FlxTextBorderStyle.OUTLINE;
                instructionText.borderColor = 0xFF000000;
                editSubState.add(instructionText);
                
                if (mobileControls.touchPad != null)
                {
                        mobileControls.touchPad.solid = true;
                        mobileControls.touchPad.draggable = true;
                }
                
                if (mobileControls.hitbox != null)
                {
                        mobileControls.hitbox.solid = true;
                        mobileControls.hitbox.draggable = true;
                }
                
                editSubState.closeCallback = function() {
                        if (mobileControls.touchPad != null)
                        {
                                MobileConfig.setPadX(mobileControls.touchPad.x);
                                MobileConfig.setPadY(mobileControls.touchPad.y);
                        }
                        
                        if (mobileControls.touchPad != null)
                                mobileControls.touchPad.draggable = false;
                        if (mobileControls.hitbox != null)
                                mobileControls.hitbox.draggable = false;
                        
                        paused = false;
                        persistentUpdate = true;
                        
                        MobileUtils.showToast("Posição salva!", false);
                };
